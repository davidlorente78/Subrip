...
namespace ReferenceIndexCreator
{
    public  class Tools
    {
        ...
        public Report stringIndex(int Level,bool acumulativo) {
            ...
            foreach (Tuple<string, List<int>> item in index)
            {
                string pages = "";
                ...
                if (items.Count() > MAXREPETITIONS)
                {
                    repetitions = repetitions + item.Item1 + "\n";
                }
            }

            Report r = new Report() { Included = con, NonIncluded = sin, NonIncludedWords = palabrassinfrases, ExtremelyReapeted = repetitions, ReportFormatted= tocopypaste };
